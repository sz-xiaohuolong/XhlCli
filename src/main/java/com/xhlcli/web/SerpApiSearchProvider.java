...
public class SerpApiSearchProvider implements SearchProvider {
    ...
    public String unavailableHint() {
        return "Web 搜索未配置 SerpAPI Key。请在环境变量或 .env 中设置 SERPAPI_KEY。";
    }
    ...
    List<SearchResult> parse(String json, int maxResults) throws IOException {
        ...
        if (organic.isArray()) {
            ...
            for (JsonNode node : organic) {
                ...
                position++;
                results.add(SearchResult.of(position, title, link, snippet, date));
            }
        }
        if (results.isEmpty()) {
            ...
            if (!answerBox.isMissingNode()) {
                ...
                if (snippet.isBlank()) snippet = answerBox.path("answer").asText("");
                if (!snippet.isBlank()) {
                    results.add(SearchResult.of(1, "Google 精选摘要", "", snippet));
                }
            }
        }
        return results;
