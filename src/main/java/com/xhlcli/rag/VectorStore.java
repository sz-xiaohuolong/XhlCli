...
/**
 * SQLite 向量存储 + 代码关系图谱与增量文件哈希持久化
 * <p>
 * 向量以 JSON 数组形式存储在 SQLite 中，检索时在内存中计算余弦相似度。
 */
public class VectorStore implements AutoCloseable {
    ...
    public VectorStore(String projectPath) throws SQLException {
        ...
        String dbDir = System.getProperty("xhlcli.rag.dir",
                System.getProperty("user.home") + "/.xhlcli/rag");
    ...
    private void initTables() throws SQLException {
        ...
        // 索引加速查询
        String[] indices = {
                "CREATE INDEX IF NOT EXISTS idx_chunks_proj ON code_chunks(project_path)",
                ...
                "CREATE INDEX IF NOT EXISTS idx_rel_proj ON code_relations(project_path)",
                "CREATE INDEX IF NOT EXISTS idx_rel_from ON code_relations(from_name)",
                "CREATE INDEX IF NOT EXISTS idx_rel_to ON code_relations(to_name)",
                "CREATE INDEX IF NOT EXISTS idx_files_proj ON code_files(project_path)",
                "CREATE INDEX IF NOT EXISTS idx_files_proj_path ON code_files(project_path, file_path)"
        };
    ...
    public void clearProject() throws SQLException {
        ...
        String deleteFiles = "DELETE FROM code_files WHERE project_path = ?";
        ...
        try (PreparedStatement ps1 = connection.prepareStatement(deleteChunks);
             ...
             PreparedStatement ps3 = connection.prepareStatement(deleteFiles)) {
    ...
    /**
     * 删除特定文件的代码块、关系以及文件索引记录
     */
    public void deleteFile(String filePath) throws SQLException {
        String delChunks = "DELETE FROM code_chunks WHERE project_path = ? AND file_path = ?";
        String delRelations = "DELETE FROM code_relations WHERE project_path = ? AND (from_file = ? OR to_file = ?)";
        String delFiles = "DELETE FROM code_files WHERE project_path = ? AND file_path = ?";

        boolean autoCommit = connection.getAutoCommit();
        connection.setAutoCommit(false);
        try (PreparedStatement ps1 = connection.prepareStatement(delChunks);
             PreparedStatement ps2 = connection.prepareStatement(delRelations);
             PreparedStatement ps3 = connection.prepareStatement(delFiles)) {
            ps1.setString(1, projectPath);
            ps1.setString(2, filePath);
            ps1.executeUpdate();
    ...
    public String getFileHash(String filePath) throws SQLException {
        String sql = "SELECT content_hash FROM code_files WHERE project_path = ? AND file_path = ?";
     ...
     * 获取当前项目已索引的所有文件路径
     */
    public Set<String> getAllIndexedFiles() throws SQLException {
        Set<String> files = new HashSet<>();
        String sql = "SELECT file_path FROM code_files WHERE project_path = ?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, projectPath);
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
     ...
     * 写入或更新文件哈希记录
     */
    public void updateFileHash(String filePath, String contentHash) throws SQLException {
        String sql = """
                INSERT INTO code_files (project_path, file_path, content_hash, last_indexed_at)
    ...
    public IndexStats getStats() throws SQLException {
        ...
        String fileSql = "SELECT COUNT(*) FROM code_files WHERE project_path = ?";
        int chunks = 0;
        int relations = 0;
        int files = 0;
        ...
        try (PreparedStatement ps = connection.prepareStatement(fileSql)) {
            ps.setString(1, projectPath);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) files = rs.getInt(1);
            }
        }
        return new IndexStats(chunks, relations, files);
    }
    ...
    public record IndexStats(int chunkCount, int relationCount, int fileCount) {
        public IndexStats(int chunkCount, int relationCount) {
            this(chunkCount, relationCount, 0);
        }
    }
}
