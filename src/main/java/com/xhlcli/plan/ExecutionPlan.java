...
public class ExecutionPlan {
    ...
    /**
     * 计算拓扑排序执行顺序（DFS + 三色标记，自带环检测）。
     *
     * 三色标记（用两个 HashSet 模拟白/灰/黑三色）：
     *   白色：两个集合都没有 → 还没访问过，需要发起 DFS
     *   灰色：在 visiting 里     → 正在递归链上、还没返回（DFS 进行中）
     *   黑色：在 visited 里     → 已处理完，结果已写入 executionOrder
     *
     * 保证核心：后序遍历——先排好【依赖】，再排【自己】，从而产出的顺序
     * 永远满足「每个任务出现时其依赖都已出现过」，即可直接按序顺序执行。
     *
     * @return true 拓扑排序成功；false 图中存在环（任务互相依赖，无解）
     */
    public boolean computeExecutionOrder() {
        executionOrder.clear();
        Set<String> visited = new HashSet<>();   // 黑色：已完成排序的任务
        Set<String> visiting = new HashSet<>();  // 灰色：正在 DFS 递归链上的任务

        for (Task task : tasks.values()) {
            if (!visited.contains(task.getId())) {
                if (!topologicalSort(task, visited, visiting)) {
                    return false;  // 有环：计划不可执行
                }
            }
        }
    ...
    private boolean topologicalSort(Task task, Set<String> visited, Set<String> visiting) {
        ...
        // 环检测：当前节点还在灰色递归链上就再次遇到 = 沿着链回到了它自己
        if (visiting.contains(id)) {
            return false;  // 有环
        }
        // 已完成：黑色节点无需重复处理，直接返回 true
        ...
        // 对每个依赖递归拓扑排序：确保「依赖」先于「自己」被排进 executionOrder
        for (String depId : task.getDependencies()) {
            Task dep = tasks.get(depId);
            if (dep != null) {
                if (!topologicalSort(dep, visited, visiting)) {
                    return false;  // 依赖链里发现了环，向上传播失败
                }
            }
        }

        // 所有依赖已归位：褪灰标黑，把自己写入执行顺序
        visiting.remove(id);
        visited.add(id);
