    ...
    void cliFormatIncludesReadableSummaryBeforeResults() {
        List<VectorStore.SearchResult> results = List.of(
                new VectorStore.SearchResult(
                        "/Users/test/xhlcli/src/main/java/com/xhlcli/agent/ReactAgent.java",
                        "method",
                        "ReactAgent.run(String userInput)",
        ...
        assertTrue(output.contains("最相关的入口是 [method:ReactAgent.run(String userInput)]"));
        assertTrue(output.contains("1. [method:ReactAgent.run(String userInput)]"));
    }
}
