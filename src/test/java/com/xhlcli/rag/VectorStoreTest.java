...
class VectorStoreTest {
    ...
    private static final String TEST_PROJECT = "/tmp/test-project-xhlcli";

    @BeforeEach
    void setUp() throws Exception {
        System.setProperty("xhlcli.rag.dir", "/tmp/xhlcli-test-rag");
    ...
    @Test
    void testIncrementalFileTracking() throws Exception {
        store.updateFileHash("A.java", "hash1");
        store.updateFileHash("B.java", "hash2");

        assertEquals("hash1", store.getFileHash("A.java"));
        assertEquals("hash2", store.getFileHash("B.java"));
        assertNull(store.getFileHash("C.java"));

        Set<String> files = store.getAllIndexedFiles();
        assertTrue(files.contains("A.java"));
        assertTrue(files.contains("B.java"));

        store.deleteFile("A.java");
        assertNull(store.getFileHash("A.java"));
        assertEquals(1, store.getAllIndexedFiles().size());
    }
}
