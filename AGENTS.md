...
- 当前阶段：Phase 13 Web 检索与浏览器集成已交付；下一阶段为 Phase 14 跨会话交互与工作区快照恢复。
- 当前已实现：Java 21 Maven 工程、可执行 JAR、DeepSeek 流式对话、进程内多轮历史、基础聊天命令、结构化 Tool Call/Observation ReAct 循环、Ctrl+C 取消、安全配置与脱敏、错误分类、统一 `RunEvent`，9 个本地开发工具 (`list_dir`, `read_file`, `write_file`, `apply_patch`, `git_diff`, `execute_command`, `glob_files`, `grep_code`, `search_code`)、工作区路径越界防护 `WorkspacePathResolver`、纯 Java 降级与 Ripgrep 双搜索引擎、嵌入式 SQLite (`sqlite-jdbc`) 与 Java AST 解析 (`javaparser-core`)、基于 SHA-256 的确定性增量索引引擎 `CodeIndex`、语义与混合检索器 `CodeRetriever`、结构化摘要格式化器 `SearchResultFormatter`、Golden Set 评测集，以及系统硬策略 (`PathGuard`, `CommandGuard`)、脱敏审计日志 (`AuditLog`)、风险分级与人工审批 (`ApprovalPolicy`, `TerminalHitlHandler`)、`DefaultToolExecutor` 安全编排闭环，上下文预算管理 (`TokenBudget`)、层级上下文组装 (`ContextAssembler`)、长期记忆 (`MemoryManager`)、会话自动压缩 (`ConversationHistoryCompactor`)，CLI 指令 (`/index`, `/search`, `/search-text`, `/context`, `/compact`, `/save`, `/memory`, `/plan`, `/team`, `/model`, `/mcp`, `/browser`)，以及 Plan 核心领域模型 (`Task`, `ExecutionPlan`)、DFS 三色标记拓扑排序与环检测、Kahn 算法分层批次、`Planner` 简单短路与重规划、人机审阅处理器 (`PlanReviewInputParser`, `PlanReviewHandler`)、分层执行引擎 `PlanExecuteAgent`（日志隔离缓冲、重排熔断保护），以及有界并发执行器 `BoundedParallelExecutor`、并发资格与资源互斥检测 `ParallelEligibilityDecider` / `ResourceAccess`、批次调度器 `ParallelBatchScheduler`、乱序保序归并、超时与失败隔离、协作式取消传播、ReactAgent & PlanExecuteAgent 并发调度打通、CLI 参数 `--max-concurrency` / `--tool-timeout`，以及 Multi-Agent 协作角色体系（`TeamRole`: Planner / Worker 池 / Reviewer）、`TeamPrompts` 专职提示词、最小化上下文交接包 `HandoverPackage`、结构化质量审查与容错降级 `ReviewResult`、打回重试与熔断保护机制（最多 2 次重试）、专职独立子代理 `SubAgent`、团队编排器 `TeamOrchestrator`，以及模型能力元信息 `ModelCapabilities`、多 Provider 抽象适配（`OpenAiClient`, `AnthropicClaudeClient` 原生协议与 `AnthropicSseParser`, `OllamaClient` NDJSON 流式协议, `DeepSeekClient`）、注册中心与模型工厂 `LlmProviderRegistry`、别名映射与凭据自动探测、运行时热切换与上下文预算重算联动、不支持工具模型的硬防御拦截护栏，以及 Model Context Protocol (MCP 2024-11-05) 生态扩展（标准 JSON-RPC 2.0 序列化、`StdioMcpTransport` 子进程管道与异常隔离、`StreamableHttpMcpTransport` HTTP/SSE 传输、两级配置合并与 `${ENV}` 严格解析隔离、`McpToolAdapter` 动态命名空间注入 `mcp__{server}__{tool}`、资源管理协议与 `/mcp read`、`McpServerManager` 3 秒启动预算管理与 `list_changed` 动态热更新、HITL 中风险审批沙箱与审计脱敏），以及 Web 检索与浏览器集成子系统（SSRF 安全围栏与 Token Bucket 限流 `NetworkPolicy`、多搜索引擎统一抽象与免 Key 降级 `SearchProvider` / `SearchProviderFactory`、5MB 受限流式抓取与 Jsoup DOM Readability 语义正文转 Markdown `WebFetcher` / `HtmlExtractor`、浏览器 `ISOLATED` / `SHARED` 模式分层 `BrowserSession`、敏感页面规则匹配库 `SensitivePagePolicy`、`BrowserGuard` 敏感写操作强制单步审批与宿主已有标签页防误关保护、CDP 端口探活与标签列表提取 `BrowserConnectivityCheck`、外部工具 `web_search`, `web_fetch`, `browser_connect`, `browser_disconnect`, `browser_status` 与终端 `/browser` 控制台），及 366 项全量自动化测试 100% 绿灯。
- 当前已发布：Phase 01 `v0.2.0`，Phase 05 `v0.3.0`，Phase 06 `v0.4.0`，Phase 07 `v0.5.0`，Phase 08 `v0.6.0`，Phase 09 `v0.7.0`，Phase 10 `v0.8.0`，Phase 11 `v0.9.0`，Phase 12 `v0.10.0`，Phase 13 `v0.11.0`。
- 当前未交付：Skills 与 Phase 14–18 的其他扩展运行能力。
